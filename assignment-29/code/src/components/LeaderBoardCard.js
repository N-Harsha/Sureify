...
const LeaderBoardCard = ({
  ...
  const scoreTable = leaderboard[category]
    ...
    .map((item) => {
      const mappedItems = Object.values(item).map((i) => <td>{i}</td>);
      return <tr>{mappedItems}</tr>;
    });

  const scoreTableHeaders = leaderboard[category]
    ? Object.keys(leaderboard[category][0])?.map((item) => (
        <th>{NameMapping[item]}</th>
      ))
    : null;

  const availableOptions = Object.keys(leaderboard).map((item) => {
